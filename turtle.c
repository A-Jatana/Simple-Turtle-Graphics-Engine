...
CmdNode *newCmdNode(char cmd[10], int val) {
   ...
   */
   ...
   if(strcmp(cmd, "penup") == 0 || strcmp(cmd, "pendown") == 0 || strcmp(cmd, "forward") == 0 || strcmp(cmd, "backward") == 0 || strcmp(cmd, "colour") == 0 || strcmp(cmd, "right") == 0 || strcmp(cmd, "left") == 0)
   {
	   strcpy(new_node->cmd, cmd);
	   new_node->val = val; //check the "ignore" part with someone
	   new_node->next = NULL;
	   return new_node;
   }
...
void printCommandList(CmdNode *head) {
   ...
   */
   ...
   while(p != NULL)
   {
	   ...
	   else if(strcmp(p->cmd, "penup") == 0 || strcmp(p->cmd, "left") == 0 || strcmp(p->cmd, "right") == 0 || strcmp(p->cmd, "pendown") == 0)
	   {
		   printf("%3d: %s\n", counter, p->cmd);
		   counter++;
	   }
   ...
   */
   ...
   while(p != NULL)
   {
	   if(strcmp(p->cmd, cmd) == 0)
	   {
		  ...
		  else if(strcmp(cmd, "penup") == 0 || strcmp(cmd, "left") == 0 || strcmp(cmd, "right") == 0 || strcmp(cmd, "pendown") == 0)
		  {
		   printf("%3d: %s\n", counter, p->cmd);
		  }
	   }
...
CmdNode *insertCommand(CmdNode *head, CmdNode *new_CmdNode) {
   ...
   */
   ...
   while(p->next != NULL) //if you did p here, then p = NULL when it exits the loop
   {
	   p = p->next;
   }
...
CmdNode *insertCommandBefore(CmdNode *head, CmdNode *new_CmdNode, int cmdNum) {
   ...
   */
   ...
   int list_size = countCommands(head);
   
   if(cmdNum >= list_size)
   {
	   printf("Invalid Command Number.\n");
	   return head; //ask if we just return head
   }
   
   if(cmdNum == 0) //confirm with SN
   {
	   new_CmdNode->next = head;
	   return new_CmdNode;
   }
...
void move(int direction, int degrees, int *endX, int *endY, int pendown, int val)
{
  // If degrees = 1 or 2, multiplier = 1. -1 otherwise
  int multiplier;
...
void run(Image *im, CmdNode *head, int *endX, int *endY) {
  ...
  CmdNode *tr = head;
  int pendown = 1;
  int degrees = 1; // 1 = downwards, 2 = right, 3 = left, 4 = upwards, starting at (0,0)
  int colour = 0;
  int startX , startY;
  *(endX) = 0;
  *(endY) = 0;
  ...
  while (tr != NULL)
  {
    if (strcmp(tr->cmd, "penup") == 0)
      pendown = 0;

    else if (strcmp(tr->cmd, "pendown") == 0)
      pendown = 1;

    else if (strcmp(tr->cmd, "colour") == 0)
      colour = tr->val;

    else if (strcmp(tr->cmd, "forward") == 0)
    {
      startX = *(endX);
      startY = *(endY);
      move(1, degrees, endX, endY, pendown, tr->val);
      if (pendown == 1)
        drawLine(im, startX, startY, *(endX), *(endY), colour);
    }
    else if (strcmp(tr->cmd, "backward") == 0)
    {
      startX = *(endX);
      startY = *(endY);
      move(-1, degrees, endX, endY, pendown, tr->val);
    ...
    else if (strcmp(tr->cmd, "left") == 0)
    {
      degrees -= 1;
      if (degrees == 0)
        degrees = 4;
    }
    else if (strcmp(tr->cmd, "right") == 0)
    {
      degrees += 1;
      if (degrees == 5)
        degrees = 1;
